...
class RevSliderBase {
	...
	public function __construct($t){
		...
		self::$url_admin_ajax = site_url('c=admin&m=admin_ajax');
		self::$url_ajax_actions = self::$url_ajax . "&action=revslider_ajax_action";
		self::$url_ajax_showimage = self::$url_admin_ajax . "&action=revslider_show_image";
	...
	*/
    ...
    public static function get_svg_sets_url(){
        ...
        // unpack svg if not extracted yet
        $ci = &get_instance();
        $ci->load->library('updater', 'Updater');
        if (file_exists(RS_PLUGIN_PATH . Updater::SVG_DIR . Updater::SVG_FILE)) {
            if (unzip_file(RS_PLUGIN_PATH . Updater::SVG_DIR . Updater::SVG_FILE, RS_PLUGIN_PATH . Updater::SVG_DIR)) {
                unlink(RS_PLUGIN_PATH . Updater::SVG_DIR . Updater::SVG_FILE);
            } else {
                RevSliderFunctions::throwError(__('Failed to unzip: ') . RS_PLUGIN_PATH . Updater::SVG_DIR . Updater::SVG_FILE);
            }
        }
